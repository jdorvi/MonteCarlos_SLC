...
OUTPUTFILE = os.path.join(ROOTDIRECT, "storms_mwd"+INPUT)
OUTPUTFILE2 = os.path.join(ROOTDIRECT, "storms_raw_mwd"+INPUT)
...
def select_storms():
    ...
    storm = {"hs":float,    # Max 1-hr significant wave height (m)
             ...
             "mwd":float,   # Mean wave direction (deg)
        ...
        if DATA["WVHT"][i] > WAVE_HEIGHT:
            ...
            while current_wave_height > WAVE_HEIGHT:
                if DATA["WVHT"][running_index] > storm["hs"]:
                    ...
                    storm["mwd"] = DATA["MWD"][running_index]
                storm["length"] += 1
    ...
    if len(storms_raw) == 0:
        ...
        while i < len(storms_raw)-1:
            ...
            if  between_2_storms < DISTINCT_EVENT_LENGTH:
                ...
                storm["mwd"] = copy(storms_raw[i+max_ind]["mwd"])
...
def write_files(storms, outputfile):
    ...
    with open(outputfile, "w") as output:
        line = "    Year   Month     Day    Hour   begin  length    hsig     tps  a_hsig   a_tps     mwd\n"
        output.write(line)
        for storm in storms:
            ...
            mwd = '{:>8.2f}'.format(storms[storm]["mwd"])
            line = year+month+day+hour+begin+length+hsig+tps+a_hsig+a_tps+mwd+"\n"
            output.write(line)
